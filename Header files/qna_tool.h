...
public:
    ...
    Obj(int bc, int page, int para, string str){
        book_code = bc;
        page_no = page;
        para_no = para;
        score=0;
...
struct HeapNode {
    ...
    HeapNode(int bc, int page, int para){
        book_code = bc;
        page_no = page;
        para_no = para;
        score = 0;
...
private:
    vector<Obj*> vec;    // for each paragraph
    Dict* dictionary;    // for whole corpus
    Dict* gen_dict;      // for general csv
...
public:
    ...
    // You can add attributes/helper functions here
    Node* get_top_k_para_(string question, int k);
};